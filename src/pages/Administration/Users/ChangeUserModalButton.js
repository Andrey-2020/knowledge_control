...
/** @extends {BaseState<import('./ChangeUserModalButton').Props>} */
class State extends BaseState {
  ...
  constructor() {
    ...
    makeObservable(this, {
      ...
      setInitUserRoleValue: action.bound,
      change: action.bound,
    });
  }
  ...
  get userAffiliationFieldComponent() {
    if (this.userRole.value === ROLES.USER) {
      return (
        <Form.Item
          name="studyGroupId"
          label={i18n.t('pages.Administration.Users.ChangeUserModalButton:template.Form.studyGroupId--label')}
          rules={this.rules.studyGroupId}
        >
          <Select
            placeholder={i18n.t('pages.Administration.Users.ChangeUserModalButton:template.Form.studyGroupId--placeholder')}
    ...
    if (this.userRole.value === ROLES.TEACHER) {
      return (
        <Form.Item
          name="departmentId"
          label={i18n.t('pages.Administration.Users.ChangeUserModalButton:template.Form.departmentId--label')}
          rules={this.rules.departmentId}
        >
          <Select
            placeholder={i18n.t('pages.Administration.Users.ChangeUserModalButton:template.Form.departmentId--placeholder')}
  ...
  setInitUserRoleValue() {
    this.userRole.value = this.props.changed.role;
  }

  /**
   * @param {import('./ChangeUserModalButton').FormData} formData
   */
   change(formData) {
    ...
    .then(() => {
      ...
      this.props.onChange();
      message.success(i18n.t('pages.Administration.Users.ChangeUserModalButton:add--success'), 3);
      this.formInstance.resetFields();
    })
    .catch((error) => {
      message.error(http.parseError(
        i18n.t('pages.Administration.Users.ChangeUserModalButton:add--error'), error), 5);
    })
 ...
 * 
 * @param {import('./ChangeUserModalButton').Props} props
 */
function ChangeUserModalButton(props) {
  const { t } = useTranslation(
    'pages.Administration.Users.ChangeUserModalButton',
    { useSuspense: false },
  );
  ...
  useOnce(state.setInitUserRoleValue)
...
export default observer(ChangeUserModalButton);
